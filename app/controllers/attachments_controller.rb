...
class AttachmentsController < ApplicationController
  ...
  def manage
    ...
    logger.debug('Attachments retrieve from ip: ' + request.remote_ip)
    @attachments = "#{params[:media]}".classify.constantize.paginate :page => params[:page], :order => 'created_at DESC', :per_page => 10, :conditions => [ "user_ip = ? and created_at > ? ", request.remote_ip, 7.days.ago]
